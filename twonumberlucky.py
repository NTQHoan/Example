...
-   Check the lotto result
    -   Case with arguments, check by 2 last digits
-   run command under the following pattern:
...
def get_number():
    ...
    lucky_number = BS(req.text, 'html.parser').find(
...
def check_number(numbers):
    ...
    for number in numbers:
        if number in lo_numbers:
            ...
            if count >= 1:
                print(
                    "number lucky: {} ,number of time: {}".format(
                        str(number), count))
            else:
